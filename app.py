...
<style>
    ...
    .stApp {
        ...
        overflow-x: hidden;
        background:
            radial-gradient(circle at 8% 8%, rgba(21,154,140,.20), transparent 24%),
            radial-gradient(circle at 92% 8%, rgba(242,184,75,.18), transparent 22%),
            radial-gradient(circle at 78% 78%, rgba(112,91,214,.10), transparent 26%),
            linear-gradient(135deg, #f7fbfa 0%, #f5f7fc 48%, #fffaf0 100%);
        color: var(--ink);
    }
    ...
    .stApp::before {
        ...
        top: 8%; right: -170px;
        background: radial-gradient(circle, rgba(21,154,140,.14), rgba(21,154,140,0) 68%);
        box-shadow: -520px 520px 0 70px rgba(242,184,75,.045);
    }
    .stApp::after {
        width: 300px; height: 300px;
        bottom: -130px; left: -120px;
        background: radial-gradient(circle, rgba(112,91,214,.10), rgba(112,91,214,0) 70%);
    }
    .main .block-container { position: relative; z-index: 1; }
    [data-testid="stHeader"] { background: rgba(247,251,250,.62); backdrop-filter: blur(14px); }
    ...
    [data-testid="stSidebar"] [data-baseweb="select"] svg { color: #173f52 !important; }
    [data-testid="stSidebar"] .stNumberInput button:hover { background: #ffffff !important; }
    .plot-container { border-radius: 18px !important; }
    ...
    /* V2 visual refinement: refined wellness cards, readable controls, and richer hierarchy */
    .main .block-container { max-width: 1500px !important; padding-left: 2.2rem !important; padding-right: 2.2rem !important; }
    .stApp { background: radial-gradient(circle at 0% 0%, rgba(61, 201, 170, .09), transparent 24%), radial-gradient(circle at 100% 0%, rgba(255, 190, 92, .11), transparent 25%), linear-gradient(135deg, #f8fbfd 0%, #f3f8f7 100%); }
    [data-testid="stSidebar"] { background: linear-gradient(180deg, #0b3b43 0%, #0d5558 52%, #116f68 100%) !important; }
    [data-testid="stSidebar"] section { padding-top: 1rem; }
    [data-testid="stSidebar"] [data-testid="stMarkdownContainer"] p { color: #f6fffc !important; }
    [data-testid="stSidebar"] .stTextInput input, [data-testid="stSidebar"] .stNumberInput input { font-size: .95rem !important; }
    [data-testid="stSidebar"] [data-baseweb="select"] * { color: #173f52 !important; -webkit-text-fill-color: #173f52 !important; }
    [data-testid="stSidebar"] .stButton > button, [data-testid="stSidebar"] .stFormSubmitButton > button { min-height: 44px !important; border-radius: 14px !important; }
    .brand-header { background: linear-gradient(120deg, #e9fbf4 0%, #ffffff 56%, #fff2d9 100%) !important; border: 1px solid #cfe9e0 !important; box-shadow: 0 18px 45px rgba(16, 70, 76, .09) !important; }
    .page-hero { background: linear-gradient(120deg, #ffffff 0%, #eefaf7 70%, #fff6e4 100%) !important; border: 1px solid #d7ebe5 !important; box-shadow: 0 14px 35px rgba(16, 70, 76, .07) !important; }
    .page-title { font-size: 1.55rem !important; }
    [data-testid="stMetric"] { border: 1px solid #deebed !important; border-top: 4px solid #21ad96 !important; background: linear-gradient(145deg, #ffffff, #f8fcfb) !important; box-shadow: 0 12px 28px rgba(16, 70, 76, .07) !important; }
    [data-testid="stMetricLabel"] { font-size: .88rem !important; color: #5c7782 !important; }
    [data-testid="stMetricValue"] { font-size: 1.65rem !important; }
    .macro-panel { border: 1px solid #dcebe8; border-radius: 24px; padding: 18px 20px; background: linear-gradient(145deg, rgba(255,255,255,.98), rgba(242,251,248,.98)); box-shadow: 0 14px 34px rgba(16,70,76,.07); }
    .macro-heading { color: #103b4b; font-size: 1.08rem; font-weight: 850; margin-bottom: 4px; }
    .macro-subheading { color: #78909b; font-size: .86rem; margin-bottom: 16px; }
    .macro-item { display:flex; align-items:center; gap:12px; padding:13px 14px; margin:10px 0; border:1px solid #e3eeec; border-radius:16px; background:#ffffff; box-shadow:0 5px 15px rgba(16,70,76,.035); }
    .macro-dot { width:12px; height:12px; border-radius:50%; flex:0 0 12px; }
    .macro-name { color:#173f52; font-weight:800; font-size:.92rem; }
    .macro-meta { color:#718793; font-size:.79rem; margin-top:2px; }
    .macro-percent { margin-left:auto; color:#087f73; font-weight:900; font-size:1rem; white-space:nowrap; }
    .macro-total { margin-top:14px; padding:14px 16px; border-radius:16px; background:linear-gradient(135deg,#e8f8f2,#fff7e6); border:1px solid #d9eade; color:#234c55; }
    .macro-total strong { color:#087f73; }
    .stPlotlyChart { border-radius: 22px; overflow:hidden; }
    [data-testid="stTabs"] [role="tablist"] { box-shadow: 0 6px 18px rgba(16,70,76,.04); }
    .stButton > button { border-radius: 14px !important; }
    .stTextInput input:focus, .stNumberInput input:focus, .stTextArea textarea:focus { border-color:#20aa95 !important; box-shadow:0 0 0 2px rgba(32,170,149,.12) !important; }
    .stDataFrame { background:#ffffff; }
    @media (max-width: 900px) { .main .block-container { padding-left: 1rem !important; padding-right: 1rem !important; } .macro-panel { margin-top: 12px; } }
...
GEMINI_MODEL = read_secret("GEMINI_MODEL", "gemini-3.6-flash")
...
if not SUPABASE_URL or not SUPABASE_KEY:
    st.error("SUPABASE_URL dan SUPABASE_KEY belum dikonfigurasi. Tambahkan pada .env atau Streamlit Secrets.")
    st.stop()
...
def validated_indonesia_match(query, limit=12):
    """Return dataset matches only when meaningful food words overlap strongly.

    Generic words such as ``segar`` are deliberately ignored so that a query
    like ``salad sayur segar`` cannot be mapped to ``ikan sale segar``.
    """
    catalog = indonesia_catalog_only()
    if catalog.empty:
        return catalog
    ...
    generic_tokens = {
        "segar", "fresh", "matang", "masak", "goreng", "rebus",
        "kukus", "bakar", "panggang", "manis", "asin", "pedas",
        "besar", "kecil", "sedang", "dengan", "tanpa", "dan",
    }
    meaningful = [t for t in query_tokens if t not in generic_tokens]
    if not meaningful:
    ...
    for idx, row in catalog.iterrows():
        ...
        overlap = set(meaningful).intersection(name_tokens)
        phrase_match = normalize_text(query) in name or name in normalize_text(query)
        coverage = len(overlap) / max(len(set(meaningful)), 1)
        # Require either a full phrase, or at least half of the meaningful
        # tokens and never accept a match based on one generic word alone.
        if phrase_match or (len(overlap) >= 2 and coverage >= 0.5) or (len(meaningful) == 1 and len(overlap) == 1):
            score = (100 if phrase_match else 0) + len(overlap) * 20 + coverage * 10
            rows.append((idx, score))

    if not rows:
        return catalog.head(0)
    ranked = pd.DataFrame(rows, columns=["_idx", "_score"]).sort_values("_score", ascending=False).head(limit)
    result = catalog.loc[ranked["_idx"].tolist()].copy()
    result["_match_score"] = ranked.set_index("_idx")["_score"]
    return result.sort_values("_match_score", ascending=False).drop(columns="_match_score")
...
def analyze_corrected_food(image_bytes, mime_type, corrected_name, corrected_grams):
    """Ask Gemini to reassess the same image using the user's correction.

    The response is an estimate for the complete corrected portion, not a
    dataset lookup. It is used as a fallback when no trustworthy dataset
    match exists.
    """
    if not GEMINI_API_KEY:
    ...
    try:
        ...
        client = genai.Client(api_key=GEMINI_API_KEY)
        prompt = f"""
Analisis ulang gambar makanan berikut berdasarkan koreksi pengguna.
Nama makanan yang diminta pengguna: {corrected_name}
Berat porsi yang diminta pengguna: {float(corrected_grams):.2f} gram

Kembalikan HANYA JSON valid dengan struktur:
{{
  "food_name": "nama makanan yang dianalisis",
  "estimated_grams": {float(corrected_grams):.2f},
  "calories": number,
  "protein_g": number,
  "fat_g": number,
  "carbs_g": number,
  "confidence": number between 0 and 1,
  "notes": "catatan singkat tentang asumsi dan ketidakpastian"
}}

Jangan mengganti nama makanan pengguna dengan makanan lain yang tidak relevan.
Jika gambar tidak cukup mendukung nama tersebut, tetap gunakan nama pengguna
dan berikan estimasi konservatif dengan catatan ketidakpastian. Nilai nutrisi
harus untuk seluruh porsi {float(corrected_grams):.2f} gram, bukan per 100 gram.
"""
        response = client.models.generate_content(
            model=GEMINI_MODEL,
            contents=[types.Part.from_bytes(data=image_bytes, mime_type=mime_type or "image/jpeg"), prompt],
        )
        raw = re.sub(r"^```(?:json)?\s*", "", (response.text or "").strip(), flags=re.IGNORECASE)
        raw = re.sub(r"\s*```$", "", raw)
        parsed = json.loads(raw)
        nutrients = {
        ...
        if warnings:
            return None, "; ".join(warnings)
        return {"food_name": corrected_name.strip(), "grams": float(corrected_grams), "nutrients": nutrients, "confidence": parsed.get("confidence"), "notes": parsed.get("notes")}, None
    except Exception as exc:
        return None, f"Analisis ulang AI gagal: {exc}"


def trigger_corrected_reanalysis():
    """Automatically re-analyze when the user confirms a name with Enter.

    The original image remains the evidence source. The corrected name and
    weight are instructions for the second AI pass; the dataset is evaluated
    afterward only as a validated nutritional reference.
    """
    corrected_name = str(st.session_state.get("ai_corrected_name", "")).strip()
    corrected_grams = st.session_state.get("ai_corrected_grams", 100.0)
    image_bytes = st.session_state.get("image_bytes", b"")
    mime_type = st.session_state.get("image_mime_type", "image/jpeg")

    if not corrected_name or not image_bytes:
        st.session_state["corrected_ai_error"] = "Nama makanan dan gambar diperlukan untuk analisis ulang."
        st.session_state.pop("corrected_ai_result", None)
        return

    with st.spinner("AI sedang menyesuaikan hasil berdasarkan koreksi kamu..."):
        ai_result, ai_error = analyze_corrected_food(
            image_bytes, mime_type, corrected_name, corrected_grams
        )
    if ai_error:
        st.session_state["corrected_ai_error"] = ai_error
        st.session_state.pop("corrected_ai_result", None)
    else:
        st.session_state["corrected_ai_result"] = ai_result
        st.session_state.pop("corrected_ai_error", None)
    st.session_state["correction_reanalysis_signature"] = (
        corrected_name.lower(), round(float(corrected_grams), 2)
    )
    ...
    if meals.empty:
        ...
        if total_macro_kcal <= 0:
            ...
            with chart_col:
                ...
                st.markdown('<div class="macro-heading">Peta komposisi energi</div><div class="macro-subheading">Proporsi setiap makronutrien dari total energi yang tercatat.</div>', unsafe_allow_html=True)
                st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
                st.markdown('</div>', unsafe_allow_html=True)
            with detail_col:
                st.markdown('<div class="macro-panel">', unsafe_allow_html=True)
                st.markdown('<div class="macro-heading">Rincian asupan hari ini</div><div class="macro-subheading">Lihat kontribusi energi dan persentase setiap nutrisi.</div>', unsafe_allow_html=True)
                for _, row in macro_df.iterrows():
                    name = row["Makronutrien"]
                    color = macro_colors[name]
                    st.markdown(
                        f'<div class="macro-item"><span class="macro-dot" style="background:{color}"></span><div><div class="macro-name">{name}</div><div class="macro-meta">{row["Energi (kkal)"]:.1f} kkal</div></div><div class="macro-percent">{row["Persentase"]:.1f}%</div></div>',
                        unsafe_allow_html=True,
                    )
                st.markdown(
                    f'<div class="macro-total"><strong>Total energi:</strong> {daily_totals["calories"]:.0f} kkal<br><span style="font-size:.86rem;color:#718793">Target harian: {float(calorie_target):.0f} kkal</span></div>',
                    unsafe_allow_html=True,
                )
                st.markdown('</div>', unsafe_allow_html=True)
...
with log_tab:
    ...
    if method == "Input manual":
        ...
        if result:
            ...
            st.caption(
                "Koreksi otomatis: tekan **Enter** setelah mengubah nama makanan. "
                "AI akan menganalisis ulang gambar menggunakan nama dan berat terbaru."
            )
            st.text_input(
                "Koreksi nama makanan",
                value=str(result.get("food_name_id") or result.get("food_name_en") or ""),
                key="ai_corrected_name",
                on_change=trigger_corrected_reanalysis,
                help="Ketik nama yang benar lalu tekan Enter untuk memulai analisis ulang AI.",
            )
            ...
            # Recompute the dataset reference using the current corrected name
            # after the widgets have been rendered.
            corrected_query = str(st.session_state.get("ai_corrected_name", "")).strip()
            correction_candidates = validated_indonesia_match(corrected_query, limit=12)
            selected_row = correction_candidates.iloc[0] if not correction_candidates.empty else None
            ...
            if corrected_ai_result:
                ...
                st.dataframe(
                    pd.DataFrame([corrected_ai_result["nutrients"]]).round(2),
                    use_container_width=True, hide_index=True
                )
                if corrected_ai_result.get("confidence") is not None:
                    st.caption(
                        f"Keyakinan analisis ulang AI: "
                        f"{float(corrected_ai_result['confidence']) * 100:.0f}%"
                    )
                if corrected_ai_result.get("notes"):
                    st.caption(f"Catatan AI: {corrected_ai_result['notes']}")

                if corrected_dataset_nutrients is not None:
                    st.markdown("#### Referensi nutrisi dari dataset")
                    st.dataframe(
                        pd.DataFrame([corrected_dataset_nutrients]).round(2),
                        use_container_width=True, hide_index=True
                    )
                    st.info(
                        "Dataset ditemukan sebagai referensi untuk nama makanan yang dikoreksi. "
                        "Nilai dataset digunakan sebagai acuan penyimpanan karena kecocokannya tervalidasi."
                    )
                else:
                    st.info(
                        "Tidak ada kecocokan dataset yang cukup kuat. "
                        "Hasil analisis ulang AI akan digunakan sebagai hasil final."
                    )

            # Dataset is used only when a meaningful match exists; otherwise
            # the second AI analysis is the sole source of saved nutrition.
            nutrients_to_save = None
            source_to_save = None
            if corrected_ai_result:
                if corrected_dataset_nutrients is not None:
                    nutrients_to_save = corrected_dataset_nutrients
                    source_to_save = "Dataset Nutrition Indonesia (referensi tervalidasi)"
                else:
                    nutrients_to_save = corrected_ai_result.get("nutrients")
                    source_to_save = "AI re-analysis berdasarkan koreksi pengguna"

            if nutrients_to_save:
                st.caption("Sumber yang akan disimpan: " + source_to_save)
                if st.button("Konfirmasi dan simpan hasil koreksi", key="save_corrected_ai"):
                    ...
                    if warnings:
                        ...
                        try:
                            save_meal(
                                user_id, selected_date, ai_meal_type, corrected_query or "Makanan AI",
                                corrected_grams, nutrients_to_save, source_to_save,
                            )
                            st.success("Hasil koreksi berhasil disimpan.")
                            for key in [
                                "image_result", "image_bytes", "image_mime_type",
                                "corrected_ai_result", "corrected_ai_error",
                                "correction_reanalysis_signature",
                            ]:
                                st.session_state.pop(key, None)
                            st.rerun()
                        except Exception as exc:
                ...
                st.warning(
                    "Masukkan koreksi nama makanan lalu tekan Enter agar AI melakukan analisis ulang. "
                    "Hasil belum dapat disimpan sebelum analisis ulang selesai."
                )
